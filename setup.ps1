...
param(
    ...
    # NuGet versions to build with. 'latest' takes the newest version that fits
    # Android; the interface lists the choices.
    [string] $DotNet = 'latest',
    [string] $Android = 'latest',
    [string] $PowerShell = 'latest',
    ...
    [string] $CacheDirectory = (Join-Path ([System.IO.Path]::GetTempPath()) 'pwsh-setup'),
    ...
    # Release: built by this script. Debug: needs the .NET SDK build, which
    # this script does not perform yet.
...
Remove-Variable -Name Step -Force
...
OPTIONS
  ...
  -Step <n|a-b|a,b>         Run steps: 5, 1-5, 4-5, or 2,3. Each step runs the
                            ...
                            build.
                              ...
                              6  Wrap the store in an AArch64 ELF64 library
  ...
  -CacheDirectory <path>    Download folder. Default: <user temp>\pwsh-setup.
  -OutputDirectory <path>   Set the directory emitted artifacts are written to.
  -Architecture <arm64-v8a> Select the target ABI.
  -Configuration <Release|Debug>
                            Release is built by this script. Debug needs
                            the .NET SDK build and is not available yet.
  ...
  -DotNet <version|latest>  .NET runtime pack for Android.
  -Android <version|latest> .NET for Android CoreCLR host and bindings.
  -PowerShell <version|latest>
                            System.Management.Automation and PowerShell SDK.
                            latest picks the newest version that fits the
                            Android API level (37 = .NET 11 = PowerShell 7.7).
...
EXECUTION CONTRACT
  ...
  --delete-packages, --output-directory <path>.
'@
}
...
$script:StepGraph = @{
    ...
    6  = @{
        ...
        Title = 'Wrap the store in an AArch64 ELF64 library'
...
# No versions and no hashes are written here. Step 2 resolves them from the
# feed on every run:
#   Channel     packages that ship together. A channel takes the newest version
#               published for all of its packages, or the version asked for
#               with -DotNet, -Android, or -PowerShell.
...
$script:PackageManifest = @(
    [ordered]@{ Id = 'Microsoft.NETCore.App.Runtime.android-arm64'; Feed = 'NuGet.org'; Channel = 'DotNet' }
    # The CoreCLR host for Android. Ships libnet-android.release.so, which the
    # APK carries as the runtime's native entry point.
    [ordered]@{ Id = 'Microsoft.Android.Runtime.CoreCLR.37.android-arm64'; Feed = 'NuGet.org'; Channel = 'Android' }
...
$script:RepositoryLibBaseUrl = 'https://raw.githubusercontent.com/MansfieldPlumbing/Pwsh/83d015b8ac6702d341f11f0b7f474b401b4bbe77/staging/lib/'
$script:LibRootManifestPath = 'manifest.json'
$script:LibRootManifestSha256 = '43A8817E503BF5FB90896041CBEFC20B7B88A933841C97076E8D36E5E1543841'
...
function Get-LibFileBytes {
    ...
    $fullPath = Join-Path $script:LibDirectory $Path
    $bytes = $null
    ...
    if (Test-Path -LiteralPath $fullPath -PathType Leaf) {
        ...
        if ($actual -cne $Sha256) {
            throw "lib source '$Path' is present but failed integrity verification. Expected $Sha256, computed $actual."
        }
        return $bytes
    }
...
function Get-LeanAssemblyManifest {
    ...
    if ($names.Count -ne 95) {
        throw "The lean assembly order must contain exactly 95 entries; it contains $($names.Count)."
    }
...
function Invoke-AcquisitionStep {
    ...
    # Channels first: the newest common version, or the one requested.
    ...
    foreach ($channel in $script:Channels) {
        $version = if ($requested[$channel] -and $requested[$channel] -ne 'latest') { $requested[$channel] } else { $available[$channel][0] }
        if ($available[$channel] -notcontains $version) {
            throw "$channel $version is not published for every package in the channel. Newest is $($available[$channel][0])."
        }
    ...
    $recordDirectory = Join-Path $OutputDirectory $Architecture
    $recordPath = Join-Path $recordDirectory 'packages.json'
    if ($PSCmdlet.ShouldProcess($recordPath, 'Write package record')) {
        [System.IO.Directory]::CreateDirectory($recordDirectory) | Out-Null
        $record = @($script:PackageManifest | ForEach-Object { [ordered]@{ Id = $_.Id; Version = $_.Version; Sha512 = $_.Sha512 } })
        ConvertTo-Json -InputObject $record -Depth 4 | Set-Content -LiteralPath $recordPath -Encoding utf8
    }
...
function Get-PayloadCandidateRank {
    ...
    if ($path.StartsWith('runtimes/android-arm64/lib/', [StringComparison]::OrdinalIgnoreCase)) { return 0 }
...
function New-ClrIf {
    param(
        [Parameter(Mandatory)][Linq.Expressions.Expression] $Test,
        [Parameter(Mandatory)][Linq.Expressions.Expression] $Then,
        [Parameter(Mandatory)][Linq.Expressions.Expression] $Else
    )
...
function New-ClrInvoke {
    ...
    $script:ExpressionFactories.Invoke.Invoke($null, [object[]]@(
        $Lambda, [Linq.Expressions.Expression[]]$Arguments))
}
...
function Invoke-SelectionStep {
    ...
    if ($selected.Count -ne 95) {
        throw "Lean assembly selection produced $($selected.Count) entries; exactly 95 are required."
    }

    $script:BuildContext.SelectedAssemblies = $selected
    Write-Host '[PASS] Step 4 complete: 95 ordered runtime assemblies selected in memory; reference-only images and Probe.r2r.dll rejected.' -ForegroundColor Green
}
...
function New-AssemblyStoreBytes {
    ...
    $version = [uint32]([uint32]$Contract.xaba.formatVersion -bor
        [uint32]$Contract.xaba.bit64Flag -bor
        [uint32]$Contract.xaba.arm64AbiFlag)
...
function Invoke-StoreStep {
    ...
    $outputDirectory = Join-Path $OutputDirectory $Architecture
...
function Get-ElfConstants {
    ...
    $wanted = @(
        'ET_DYN', 'EM_AARCH64', 'ELFCLASS64', 'ELFDATA2LSB',
        'EV_CURRENT', 'PT_LOAD', 'PT_DYNAMIC', 'PT_PHDR',
        'PF_R', 'PF_W', 'STB_GLOBAL', 'STT_OBJECT'
    )
    foreach ($name in $wanted) {
        $match = [regex]::Match($elfText, "\b$name\s*=\s*(\d+)")
        if (-not $match.Success) {
            throw "ELF.h does not declare '$name'."
        }
        $constants[$name] = [uint32]$match.Groups[1].Value
    }

    $tagText = Import-LibSourceText -Path 'DynamicTags.def'
    foreach ($tag in @('NULL', 'HASH', 'STRTAB', 'SYMTAB', 'STRSZ', 'SYMENT', 'SONAME')) {
        $match = [regex]::Match($tagText, "DYNAMIC_TAG\($tag,\s*(\d+)\)")
        if (-not $match.Success) {
            throw "DynamicTags.def does not declare 'DT_$tag'."
        }
        $constants["DT_$tag"] = [uint64]$match.Groups[1].Value
    }
...
function New-ElfPayloadLibrary {
    ...
    try {
        ...
        $writer.Write([uint16]$elf['EM_AARCH64'])
...
function Test-ElfPayloadLibrary {
    ...
    if ([BitConverter]::ToUInt16($bytes, 18) -ne $elf['EM_AARCH64']) { throw 'The emitted library is not EM_AARCH64.' }
...
function Invoke-NativeStep {
    ...
    if ((Get-ElfConstants)['EM_AARCH64'] -ne $expectedMachine) {
    ...
    if ($PSCmdlet.ShouldProcess($libraryPath, 'Write assembly store library')) {
        [System.IO.File]::WriteAllBytes($libraryPath, $library.Bytes)
    }
    ...
    Write-Host ('[PASS] Step 6 complete: {0} emitted as an AArch64 ET_DYN image of {1} bytes, carrying the store at offset {2} under the ''{3}'' symbol with {4} bytes of ELF overhead. Resolved through the emitted hash table exactly as dlsym would. SHA-256 {5}' -f
        $soname,
        $report.ImageSize,
...
function Invoke-DexStep {
    ...
    if ($PSCmdlet.ShouldProcess($dexPath, 'Write Java peer dex')) {
        [System.IO.File]::WriteAllBytes($dexPath, $dexBytes)
    }
...
function Invoke-AssembleStep {
    ...
    $abi = $Architecture
    $runtimePack = 'Microsoft.NETCore.App.Runtime.android-arm64'
    $hostPack = 'Microsoft.Android.Runtime.CoreCLR.37.android-arm64'
    ...
    $natives = [ordered]@{
        'libcoreclr.so'                                = @{ Package = $runtimePack; Path = 'runtimes/android-arm64/native/libcoreclr.so' }
        'libclrjit.so'                                 = @{ Package = $runtimePack; Path = 'runtimes/android-arm64/native/libclrjit.so' }
        'libSystem.Native.so'                          = @{ Package = $runtimePack; Path = 'runtimes/android-arm64/native/libSystem.Native.so' }
        'libSystem.Globalization.Native.so'            = @{ Package = $runtimePack; Path = 'runtimes/android-arm64/native/libSystem.Globalization.Native.so' }
        'libSystem.IO.Compression.Native.so'           = @{ Package = $runtimePack; Path = 'runtimes/android-arm64/native/libSystem.IO.Compression.Native.so' }
        'libSystem.Security.Cryptography.Native.Android.so' = @{ Package = $runtimePack; Path = 'runtimes/android-arm64/native/libSystem.Security.Cryptography.Native.Android.so' }
        # The Android host itself. Packaged under the name the runtime loads.
        'libmonodroid.so'                              = @{ Package = $hostPack; Path = 'runtimes/android-arm64/native/libnet-android.release.so' }
    }
...
function Invoke-SignStep {
    ...
    $certificatePath = Join-Path ([Environment]::GetFolderPath('LocalApplicationData')) 'Pwsh\pwsh-signing.pfx'
    $certificate = Get-SigningCertificate -Path $certificatePath
    ...
    $apkPath = Join-Path $outputDirectory 'Pwsh.apk'
    if ($PSCmdlet.ShouldProcess($apkPath, 'Write signed APK')) {
        [System.IO.File]::WriteAllBytes($apkPath, $signed.Bytes)
    }
    ...
    Write-Host ('[PASS] Step 11 complete: Pwsh.apk signed with APK Signature Scheme v2. {0} bytes, {1}-byte signing block, {2}-byte RSA signature, certificate {3}. Signature re-verified against the recomputed content digest. SHA-256 {4}' -f
        $signed.Bytes.Length,
        $report.BlockSize,
...
function New-ElfDataLibrary {
    ...
    try {
        ...
        foreach ($relocation in $Relocations) {
            ...
            $writer.Write([uint64]$R_AARCH64_RELATIVE)
        ...
        Write-ByteSpan -Writer $writer -Bytes ([byte[]]@(0xC0, 0x03, 0x5F, 0xD6))
...
function Test-XamarinAppLibrary {
    ...
    for ($i = 0; $i -lt $count; $i++) {
        ...
        if ($info -ne 1027) { throw "Relocation $i is type $info, not R_AARCH64_RELATIVE." }
    ...
    if ([BitConverter]::ToUInt32($bytes, $configAddress + 20) -ne 95) {
        throw 'application_config does not declare 95 assemblies.'
    }
...
function New-XamarinAppLibrary {
    ...
    # application_config. Field order is ApplicationConfigCLR.cs exactly.
    & $align 8
    $applicationConfigOffset = [int]$data.Position
    & $define 'application_config' 72
    ...
    $writer.Write([uint64]0)    # android_package_name, supplied by relocation
    $writer.Write([byte]1)      # have_assembly_store
    Write-ByteSpan -Writer $writer -Bytes (New-Object byte[] 7)   # tail padding to 72
    ...
    & $align 8
    & $define 'init_runtime_property_names' ($runtimeProperties.Count * 8)
    ...
    & $align 8
    & $define 'init_runtime_property_values' ($runtimeProperties.Count * 8)
    ...
    & $zeros 'assembly_store' 24
    & $zeros 'assembly_store_bundled_assemblies' ($AssemblyCount * 32)
    ...
    # resolver. With marshal methods disabled there is nothing to record.
    & $align 4
    & $define 'xamarin_app_init' 4 'FUNC'
    Write-ByteSpan -Writer $writer -Bytes ([byte[]]@(0xC0, 0x03, 0x5F, 0xD6))   # ret
...
function Invoke-AppDataStep {
    ...
    $report = Test-XamarinAppLibrary -Library $library -RequiredSymbols $required -PackageName $script:PackageName
...
function Show-DebugPopup {
    ...
    Show-Popup -Title 'Debug' -Lines (@(
        ...
        "config $Configuration  payload $Payload  step $Step  arch $Architecture  whatif $([bool]$WhatIfPreference)"
...
$script:ChosenVersions  = @{ DotNet = $DotNet; Android = $Android; PowerShell = $PowerShell }
...
function Get-VersionDisplay {
    ...
    if ($null -eq $script:ChannelVersions) { return $(if ($chosen -eq 'latest') { 'loading...' } else { $chosen }) }
    if ($script:ChannelVersions -is [string]) { return $(if ($chosen -eq 'latest') { 'feed unavailable' } else { $chosen }) }
    if ($chosen -eq 'latest') { return "$($script:ChannelVersions[$Channel][0]) (newest)" }
    return $chosen
}
...
function Get-NavigationItems {
    ...
    @(
        @{ Id = 'Architecture'; Kind = 'Option'; Label = 'Architecture'; Value = $Architecture; Caption = '64-bit ARM' }
        @{ Id = 'Configuration'; Kind = 'Option'; Label = 'Configuration'; Value = $Configuration
           Caption = if ($Configuration -eq 'Release') { 'setup.ps1' } else { '.NET SDK build' } }
...
function Set-OptionValue {
    ...
    switch ($Id) {
        'Configuration' {
            $script:Configuration = if ($Configuration -eq 'Release') { 'Debug' } else { 'Release' }
        }
        'Payload' {
        ...
        { $_ -in 'DotNet', 'Android', 'PowerShell' } {
            # Right steps to older versions, left back toward the newest.
            if ($script:ChannelVersions -isnot [System.Collections.IDictionary]) { return }
            $list = @($script:ChannelVersions[$Id])
            $chosen = $script:ChosenVersions[$Id]
            $at = if ($chosen -eq 'latest') { 0 } else { [Math]::Max(0, [Array]::IndexOf($list, $chosen)) }
            $at = [Math]::Min($list.Count - 1, [Math]::Max(0, $at + $Direction))
            $script:ChosenVersions[$Id] = if ($at -eq 0) { 'latest' } else { $list[$at] }
        }
    }
}
...
function Save-BuildLog {
    $directory = Join-Path $OutputDirectory $Architecture
    [System.IO.Directory]::CreateDirectory($directory) | Out-Null
    $path = Join-Path $directory ('setup-{0:yyyyMMdd-HHmmss}.log' -f (Get-Date))
    [System.IO.File]::WriteAllLines($path, $script:BuildLog)
...
function Get-CanvasCells {
    ...
    elseif ($mode -eq 'Building') {
        & $DrawText 2 4 'Building Pwsh.apk' "$script:BG_NAVY;$script:FG_ACCENT"
    ...
    if ($script:UIMode -eq 'HelpView') {
        $box = @(
            ...
            '  -DotNet <version>           .NET runtime (default latest)'
            '  -Android <version>          Android host (default latest)'
            '  -PowerShell <version>       SMA and SDK (default latest)'
...
function Request-Build {
    # Debug asks first; Release builds straight away.
    if ($Configuration -eq 'Debug') {
        Show-Popup -Title 'Debug build' -Confirm -Lines @(
            'This will initiate the .NET build setup,'
            'which Debug builds require.'
            ''
            'Are you sure?'
...
function Start-PipelineWorker {
    ...
    [void]$script:WorkerPowerShell.AddScript({
        param($EventQueue, $ScriptPath, $TargetStep, $Architecture, $Configuration, $Payload, $DotNet, $Android, $PowerShell, $Packages,
              $CacheDirectory, $OutputDirectory, $DeletePackages, $PreviewOnly)
        Set-StrictMode -Version Latest
        try {
            $arguments = @{
                Headless        = $true
            ...
            $EventQueue.Add([PSCustomObject]@{
                Type = 'Output'
                Text = "[$(Get-Date -Format 'HH:mm:ss')] setup.ps1 -Step $TargetStep -Configuration $Configuration -Payload $Payload$(if ($PreviewOnly) { ' -WhatIf' })"
            })
...
try {
    if ($Configuration -eq 'Debug') {
        throw 'Debug builds need the .NET SDK build, which this script does not perform yet. Use Release.'
    }
